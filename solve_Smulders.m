function [X,Xmod,Xmod2]=solve_Smulders(N0,Nmax,t0,tmax)
%Algorithm for solving multi-classes model on a computational domain
% [N0,Nmax] * [t0,tmax] (Lagrangian)
%using a Lax-Hopf type formula for GSOM models
%(see Costeseque-Lebacque-2014)

%Création nouveau dossier
newFolder='SimData';
mkdir(newFolder)

%Incorporer toutes les fenêtres graphiques dans la fenêtre courante
...
%Smulders(r,I)=diagramme fondamental vitesse-interdistance (r)
...
rmin=15; %interdistance minimale en mètres
rmax=50; %interdistance "maximale" en mètres
Xt0=zeros(P1+1,1);
Rt0=(rmax-rmin).*rand(P1+1,1)+rmin;
It0=floor(1.3+rand(P1+1,1)); %Initial classes (Nb_cars > Nb_trucks)
Xt0(1)=0;
for p=2:P1+1
...
%          et Rn0=@(t) invSmulders(vn0(t),I(N0));
Xn0=zeros(P2+1,1);
%--------------------------------------------------------------------------
% vmax=25;
...
Rn0=zeros(P2+1,1);
Xn0(1)=0;
Rn0(1)=invSmulders(vn0(1),It0(1));
for p=2:P2+1
    Xn0(p)=Xn0(p-1)+vn0(p-1)*Deltat;
    Rn0(p)=invSmulders(vn0(p),It0(1));
end
V0=vn0(1);
...
for p=1:P
    ...
    if p<=P1
        ...
        if p==1
            ...
            vgauche=Smulders(rgauche,Igauche);
        end
        %Déterminer le spacing projeté sur le DF de la bande considérée
        if vgauche<Smulders(inf,I(Np0))
            rstar=invSmulders(vgauche,I(Np0));
        else
            rstar=inf;
        end
        if rstar>rp0
        ...
        while Nold<Nmax && Told<tmax     
            if q==1
                ...
                for i=1:nbcaract
                    ...
                    mu(i,1)=partialSmulders(R(i,1),I(Np0));
                    T(i,2)=t0+DeltaN/mu(i,1);
                    if i==1
                        T(i,2)=t0;
                    end
                    L(i,1)=Lagrangian_Smulders(mu(i,1),I(Np0));
                ...
                for i=1:nbcaract
                    Rold=R(i,q-1);
                    if Smulders(Rold,I(Nold))<Smulders(inf,I(Nnew))
                        %on se projete sur le nouveau DF si possible
                        Rnew=invSmulders(Smulders(Rold,I(Nold)),I(Nnew));
                    else
                    ...
                    mu(i,q)=partialSmulders(Rnew,I(Nnew)); %pente
                    ...
                    L(i,q)=Lagrangian_Smulders(mu(i,q),I(Nnew));
...
% for Np0=A
%     if Np0<=Nmax
%         Xmod=LagrData2(Xmod,Np0,N0,Nmax,t0,tmax,DeltaN,Deltat,I,Rt0,newFolder,oldFolder);
%     end
% end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
